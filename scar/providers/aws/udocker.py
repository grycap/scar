...
class Udocker():
    ...
    def __init__(self, aws_properties: str, tmp_payload_folder_path: str, supervisor_zip_path: str):
    ...
    def _set_udocker_local_registry(self):
        self._aws['lambda']['environment']['Variables']['UDOCKER_REPOS'] = '/var/task/udocker/repos/'
        self._aws['lambda']['environment']['Variables']['UDOCKER_LAYERS'] = '/var/task/udocker/layers/'
    ...
    def _create_udocker_container(self):
        ...
        if self._aws.get('lambda').get('deployment').get('bucket', False):
            self._validate_container_size(self._aws.get('lambda').get('deployment').get('max_s3_payload_size'))
        else:
            self._validate_container_size(self._aws.get('lambda').get('deployment').get('max_payload_size'))

    def _validate_container_size(self, max_payload_size):
        if FileUtils.get_tree_size(self._udocker_dir) < (max_payload_size / 2):
            ucmd = self._udocker_exec + ["create", "--name=lambda_cont", self._aws.get('lambda').get('container').get('image')]
            SysUtils.execute_command_with_msg(ucmd, cli_msg="Creating container structure")
            self._aws['lambda']['environment']['Variables']['UDOCKER_CONTAINERS'] = '/var/task/udocker/containers/'
    ...
    def download_udocker_image(self):
        ...
        SysUtils.execute_command_with_msg(self._udocker_exec + ["pull", self._aws.get('lambda').get('container').get('image')],
    ...
    def prepare_udocker_image(self):
        ...
        FileUtils.copy_file(self._aws.get('lambda').get('image_file'), image_path)
        ...
        self._aws['lambda']['container']['image'] = cmd_out.split('\n')[1]
        self._create_udocker_container()
        self._set_udocker_local_registry()
        self._restore_udocker_env()
