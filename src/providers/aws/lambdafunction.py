...
class Lambda(GenericClient):
    ...
    def set_required_environment_variables(self):
        ...
        ###
        self.add_lambda_environment_variable('EXECUTION_MODE',  self.properties['execution_mode'])
        self.add_lambda_environment_variable('ROLE',  self.aws_properties['iam']['role'])
        self.add_lambda_environment_variable('MEMORY',  str(self.properties['memory']))
        ###
