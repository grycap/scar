...
class NixAuthentication(object):
    ...
    def _get_user_from_host(self, wanted_user):
        ...
        wanted_uid = ""
        if (isinstance(wanted_user, (int, int)) or
    ...
    def _get_group_from_host(self, wanted_group):
        ...
        wanted_gid = ""
        if (isinstance(wanted_group, (int, int)) or
    ...
    def add_user(self, user, passw, uid, gid, gecos,
        ...
        try:
            outpasswd = open(self.passwd_file, "a")
        except (IOError, OSError):
            return False
        else:
    ...
    def add_group(self, group, gid):
        ...
        try:
            outgroup = open(self.group_file, "a")
        except (IOError, OSError):
            return False
        else:
...
class PRootEngine(ExecutionEngineCommon):
    ...
    def _get_network_map(self):
        ...
        proot_netmap_list = []
        for (cont_port, host_port) in self._get_portsmap().items():
        ...
        if (self.cmdp.get("--help", "GEN_OPT") or
            ...
            if command in cmds:
                ...
                if isinstance(status, bool):
                    return not status
                elif isinstance(status, (int, int)):
